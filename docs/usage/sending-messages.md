...
- [Sending a Flow Message](#sending-a-flow-message)
...
The `send_flow_message` method delivers a WhatsApp Flow interactive message.
...
### Parameters
- **`recipient_phone_number`** *(str)*: The recipient's WhatsApp phone number in international format (e.g., 1234567890, without the +).
- **`body_text`** *(str)*: Body text shown in the Flow message (max 1024 characters).
- **`flow_cta`** *(str)*: CTA button text (recommended 30 characters or fewer, no emoji).
- **`mode`** *(str)*: Flow mode, `"draft"` or `"published"`. Default `"published"`.
- **`flow_action`** *(str)*: `"navigate"` or `"data_exchange"`. Default `"navigate"`.
- **`flow_id`** *(str, optional)*: Unique Flow ID from WhatsApp. Required if `flow_name` is not provided.
- **`flow_name`** *(str, optional)*: Flow name. Required if `flow_id` is not provided.
- **`header_text`** *(str, optional)*: Optional header text.
- **`footer_text`** *(str, optional)*: Optional footer text.
- **`flow_token`** *(str, optional)*: Optional flow token generated by the business. If omitted, the client generates a random token.
- **`flow_action_payload`** *(dict, optional)*: Optional payload for the flow action, e.g., `{"screen": "FIRST_ENTRY_SCREEN", "data": {...}}`.
- **`context_message_id`** *(str, optional)*: Message ID if replying within an existing thread.
- **Note:** `flow_message_version` is automatically set to `"3"` by the client.
- **Validation:** The client raises `ValueError` if neither `flow_id` nor `flow_name` is provided, if `flow_cta` exceeds 30 characters, or if `body_text` exceeds 1024 characters.

### Example: Sending a Flow Message
```python
recipient_phone_number = "1234567890"

response = client.send_flow_message(
    recipient_phone_number=recipient_phone_number,
